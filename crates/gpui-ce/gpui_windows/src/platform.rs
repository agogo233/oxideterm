...
impl Platform for WindowsPlatform {
    ...
    fn restart(&self, binary_path: Option<PathBuf>, arguments: Vec<OsString>) {
        ...
        // Spawn a waiter copy of ourselves instead of a hidden PowerShell relay:
        // the waiter blocks until this process exits, then relaunches the app
        // with the original arguments. Waiting for exit also guarantees the
        // single-instance lock is released before the new primary starts.
        //
        ...
        self.foreground_executor
            .spawn(async move {
                let mut command = new_std_command(&app_path);
                command
                    .arg(RESTART_WAIT_ARGUMENT)
                    .arg(pid.to_string())
                    .args(&arguments);
                #[allow(
                ...
                match restart_process {
                    ...
                    Err(e) => log::error!("failed to spawn restart waiter: {:?}", e),
                }
            })
            .detach();
    }
